...
def generate_identity_card(
    ...
    document_id = f"identity_card_{index:03d}"
    ...
    pdf.drawString(40, height - 50, "SPECIMEN ID CARD")
    pdf.drawString(40, height - 70, "NOT A REAL IDENTITY DOCUMENT")
    ...
    # Factuurinformatie
    purchase_order_number = fake.random_number(digits=6)
    purchase_order_date = fake.date_this_year()

    pdf.setFont("Helvetica-Bold", 10)
    pdf.drawString(350, height - 85, "Bestelgegevens")

    pdf.setFont("Helvetica", 9)
    pdf.drawString(
        350,
        height - 102,
        f"Bestelnummer: {purchase_order_number}",
    )
    pdf.drawString(
        350,
        height - 117,
        f"Besteldatum: {purchase_order_date}",
    )
    pdf.drawString(
        350,
        height - 132,
    ...
    return {
        ...
        "source": "synthetic",
        "document_type": "identity_card_specimen",
